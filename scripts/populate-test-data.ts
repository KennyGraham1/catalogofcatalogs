...
function generateEvent(region: typeof nzRegions[0], completeness: 'full' | 'partial' | 'basic'): any {
  ...
  if (completeness === 'full') {
    ...
    event.preferred_origin_id = `smi:local/origin/${Date.now()}_${randomInt(1000, 9999)}`;
    event.preferred_magnitude_id = `smi:local/magnitude/${Date.now()}_${randomInt(1000, 9999)}`;
    ...
    event.magnitudes = JSON.stringify([
      {
        publicID: event.preferred_magnitude_id,
      ...
      db.run(
        ...
        function(err) {
          ...
          for (let i = 0; i < eventCount; i++) {
            ...
            let completeness: 'full' | 'partial' | 'basic';
            ...
            const fields = ['catalogue_id', 'time', 'latitude', 'longitude', 'depth', 'magnitude'];
            const values = [catalogue.id, event.time, event.latitude, event.longitude, event.depth, event.magnitude];
            const placeholders = ['?', '?', '?', '?', '?', '?'];
