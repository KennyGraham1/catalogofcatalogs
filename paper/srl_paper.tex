...
exist---the window method of \citet{gardner1974} with \citet{uhrhammer1986}
parameters and the aftershock-zone scaling of \citet{kagan2002}, the link-based
...
bulletins \citep{chamberlain2018,illsley2025}, while every regional network
...
in isolation, available in one tool or another; what is missing is an open
platform that chains them into a single reproducible, provenance-preserving
workflow.  Table~\ref{tab:comparison} contrasts the capabilities of
representative existing tools with those of CofC.
...
ObsPy \citep{obspy2010}      & \checkmark     & ($\checkmark$) & --           & ($\checkmark$) & --           & ($\checkmark$) \\
OpenQuake \citep{pagani2014} & ($\checkmark$) & --             & --           & ($\checkmark$) & --           & --             \\
...
together with an objective event-level quality index.  Specifically, the
contributions are:
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{An integrated, browser-based workflow} spanning
    ...
    (Table~\ref{tab:comparison}).  To our knowledge no existing open tool
    covers this full chain.
  \item \textbf{An objective, reproducible event-level quality index}
    (equation~\ref{eq:quality}): a transparent five-dimension 0--100 score with
    letter grades, grounded in the established determinants of solution quality
    \citep{bondar2004,woessner2005,warrensmith2025} and re-exposed as
    user-configurable weights through the API.  We are not aware of a prior
    production tool that assigns a comparable per-event quality score.
  ...
  \item \textbf{FAIR-by-design provenance}: every merged catalogue records its
    ...
    thresholds, quality score) and re-exports it losslessly in QuakeML
    \citep{schorlemmer2011}, operationalising the FAIR principles
...
best practice \citep{datacite2021}:
\begin{itemize}[leftmargin=*]
...
scientific reusability in practice \citep{woessner2010}: missing required
...
\midrule
A$^+$ & 95--100 & Publication-ready; suitable for PSHA and all quantitative analyses \citep{gerstenberger2024}. \\
...
(50\% of Agency~B events have a GeoNet counterpart).  Concatenating the
$120\,000 + 98\,000 = 218\,000$ raw records and removing one member of each
duplicate pair yields a merged catalogue of 169\,000 unique events.
Provenance is retained: 71\,000 events are from GeoNet only, 49\,000 from
Agency~B only (events outside GeoNet's detection footprint or below its
reporting threshold), and 49\,000 from the Quality-based resolution of
duplicate pairs.
...
catalogue retains 143\,650 events (85\% of the 169\,000 unique events).
...
slightly to $0.94 \pm 0.03$ (Figure~\ref{fig:fmd})---illustrating the
...
Section~\ref{sec:example} the raw catalogue gives $\hat{b} = 1.02$ versus
$0.94$ after declustering, an upward bias of the raw value.
...
% --- Figures ---------------------------------------------------
% Floats are ordered to match first in-text citation (map, gap, FMD);
% on-disk file names retain their original generator-assigned labels.
\clearpage
\begin{figure}[ht]
  \centering
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    these clustered small events, shifting $\hat{b}$ from $1.02 \pm 0.03$
    (raw) to $0.94 \pm 0.03$ (declustered) and removing 23\% of
