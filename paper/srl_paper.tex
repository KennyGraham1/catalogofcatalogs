...
windows $\tau \leq 60$\,s and $\delta \leq 50$\,km are widened automatically for
larger and deeper events (up to roughly $3\times$ in time, $4\times$ in distance,
and $1.5\times$ for depth).  Magnitude is not part of the matching test; magnitude
consistency is instead enforced as the per-group range gate described below.
Near-antimeridian pairs use unit-vector averaging to avoid sign-wrapping errors in
the Pacific region.

The baseline windows are user-configurable in the merge UI (with strict, moderate,
and loose presets) and follow international practice for global earthquake
association \citep{storchak2013, engdahl1998}.
...
$\sigma_b = \hat{b}/\sqrt{N}$ already exceeds 0.07, comparable to the biases
...
(Section~\ref{sec:biases}).  The time-series of cumulative moment release is
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    (equations~\ref{eq:dt} and~\ref{eq:dd}).  \textit{(b)}~The merged catalogue
