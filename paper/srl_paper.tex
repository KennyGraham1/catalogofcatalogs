...
Regional monitoring agencies---GeoNet (New Zealand; \citealt{geonet2017}),
...
differ from manually reviewed ones.  \citet{bondarbullettine2004} showed
...
$\pm 0.2$ \citep{marzocchi2014}.  \citet{tinti1987} showed that even with a
...
\citet{bondarbullettine2004} demonstrated that azimuthal gap is the primary
...
At the institutional level, major agencies---GeoNet \citep{geonet2017},
...
merging across networks \citep{schorlemmer2024}, but these methods are not
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Frontend:} Next.js~14 \citep{nextjs2024} (App Router,
    TypeScript, Tailwind CSS, shadcn/ui components, Recharts for time-series,
    Leaflet \citep{leaflet2024} for interactive maps).
  \item \textbf{Backend:} Next.js API routes (serverless functions),
    MongoDB~7 \citep{mongodb2024} for persistent storage.
  \item \textbf{Parsers:} Custom TypeScript parsers for CSV/TXT, GeoJSON
    \citep{geojson2016}, and QuakeML~1.2 BED \citep{schorlemmer2011}.  FDSN
    web-service integration for live GeoNet data.
\end{itemize}

The application is containerised with Docker (Compose recipes for development
and production) and is designed to deploy to any cloud provider or on-premises
server.
...
\begin{description}[leftmargin=2em]
  ...
  \item[$Q_{\text{rel}}$ — Reliability (15\%)]  Instrumental solution
    ...
    error; \citealt{bondarbullettine2004}; \citealt{warrensmith2025}),
...
$N = 200$, the empirical minimum identified by \citet{marzocchi2014} for a
...
\citep{geonet2017, warrensmith2025} but do not represent any specific real
...
the primary predictor of horizontal location error \citep{bondarbullettine2004};
...
bias this can introduce \citep{marzocchi2014, tinti1987}, automated
